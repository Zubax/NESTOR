...
from blackstore.database import SqliteDatabase
from blackstore.rest_api import create_app

LOGGER = logging.getLogger(__name__)

DEFAULT_HOST = "0.0.0.0"
DEFAULT_PORT = 8000
DEFAULT_DB_PATH = "./blackstore.db"
DEFAULT_LOG_LEVEL = "INFO"
DEFAULT_LOG_FILE = "./blackstore.log"
DEFAULT_LOG_MAX_BYTES = 128 * 1024 * 1024
DEFAULT_LOG_BACKUP_COUNT = 20

ENV_HOST = "BLACKSTORE_HOST"
ENV_PORT = "BLACKSTORE_PORT"
ENV_DB_PATH = "BLACKSTORE_DB_PATH"
ENV_UDS = "BLACKSTORE_UDS"
ENV_ROOT_PATH = "BLACKSTORE_ROOT_PATH"
ENV_LOG_LEVEL = "BLACKSTORE_LOG_LEVEL"
ENV_LOG_FILE = "BLACKSTORE_LOG_FILE"
ENV_LOG_MAX_BYTES = "BLACKSTORE_LOG_MAX_BYTES"
ENV_LOG_BACKUP_COUNT = "BLACKSTORE_LOG_BACKUP_COUNT"
...
def parse_serve_config(argv: Sequence[str] | None = None, env: Mapping[str, str] | None = None) -> ServeConfig:
    ...
    parser = argparse.ArgumentParser(
        prog="blackstore serve",
        description="Run Blackstore CF3D REST API server",
    )
...
class _ServerTests(unittest.TestCase):
    ...
    def test_parse_serve_config_reads_environment(self) -> None:
        env = {
            ...
            ENV_UDS: "/tmp/blackstore.sock",
            ENV_ROOT_PATH: "/gw",
            ENV_LOG_LEVEL: "warning",
            ENV_LOG_FILE: "/tmp/blackstore.log",
        ...
        self.assertEqual("/tmp/blackstore.sock", config.uds)
        self.assertEqual("/gw", config.root_path)
        self.assertEqual("WARNING", config.log_level)
        self.assertEqual("/tmp/blackstore.log", config.log_file)
    ...
    def test_build_database_creates_missing_parent_directories(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            db_path = Path(temp_dir) / "nested" / "deeper" / "blackstore.sqlite3"
    ...
    def test_configure_logging_installs_stream_and_rotating_handlers(self) -> None:
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                log_file = Path(temp_dir) / "logs" / "blackstore.log"
    ...
    def test_configure_logging_uses_color_formatter_for_tty_stderr(self) -> None:
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                log_file = Path(temp_dir) / "logs" / "blackstore.log"
                config = ServeConfig(log_file=str(log_file), log_level="INFO")
                with patch("blackstore.server._should_color_stderr", return_value=True):
                    configure_logging(config)
                ...
                self.assertIn("blackstore.server", rendered)
        finally:
            for handler in list(root_logger.handlers):
                root_logger.removeHandler(handler)
                handler.close()
    ...
    def test_configure_logging_stderr_and_file_remain_plain_when_color_disabled(self) -> None:
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                log_file = Path(temp_dir) / "logs" / "blackstore.log"
                config = ServeConfig(log_file=str(log_file), log_level="INFO")
                with patch("blackstore.server._should_color_stderr", return_value=False):
                    configure_logging(config)
    ...
    def test_serve_invokes_uvicorn_with_host_port(self) -> None:
        ...
        with (
            patch("blackstore.server.configure_logging") as mocked_configure,
            patch("blackstore.server.build_database", return_value=fake_database) as mocked_build_db,
            patch("blackstore.server.create_app", return_value=fake_app) as mocked_create_app,
    ...
    def test_serve_invokes_uvicorn_with_uds(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            uds_path = Path(temp_dir) / "run" / "blackstore.sock"
            ...
            with (
                patch("blackstore.server.configure_logging"),
                patch("blackstore.server.build_database", return_value=fake_database),
                patch("blackstore.server.create_app", return_value=fake_app),
                patch.dict(sys.modules, {"uvicorn": fake_uvicorn}),
            ):
                serve(config)
    ...
    def test_create_app_from_env_builds_application(self) -> None:
        ...
        with (
            patch("blackstore.server.parse_serve_config", return_value=config) as mocked_parse,
            patch("blackstore.server.configure_logging") as mocked_logging,
            patch("blackstore.server.build_database", return_value=fake_database) as mocked_database,
            patch("blackstore.server.create_app", return_value=fake_app) as mocked_create_app,
        ):
