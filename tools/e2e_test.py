#!/usr/bin/env python3
"""
Standalone end-to-end validator for Blackstore.

Why this exists:
- The nox session should stay small and orchestration-heavy logic belongs in a dedicated script.
- The test exercises the real process boundary (`blackstore serve`) plus the real uploader tool
  (`tools/blackstore_ingest.py`) and then verifies query endpoints.
...
LOGGER = logging.getLogger("blackstore_e2e")
DEFAULT_DATASET_PATH = Path("data") / "0000000.cf3d"
DEFAULT_INGEST_SCRIPT_PATH = Path("tools") / "blackstore_ingest.py"
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="End-to-end Blackstore test: serve + ingest + retrieval")
    ...
    parser.add_argument(
        ...
        help="Path to ingest script (tools/blackstore_ingest.py)",
    )
...
def run(argv: list[str] | None = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="blackstore-e2e-") as temp_dir:
        ...
        db_path = temp_root / "blackstore-e2e.db"
        log_path = temp_root / "blackstore-e2e.log"
        stdout_path = temp_root / "blackstore-e2e.stdout.log"
        stderr_path = temp_root / "blackstore-e2e.stderr.log"
