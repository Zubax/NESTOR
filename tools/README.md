...
```bash
python tools/blackstore_ingest.py --server http://127.0.0.1:8000 --device_uid 0x123 --device "Vehicle-A" data/*.cf3d
```
...
Runs a focused startup smoke test for `blackstore serve`:
- launches a real `blackstore serve` process in a temporary environment,
